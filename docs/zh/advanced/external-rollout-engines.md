...
External rollout engine 指的是：VLlm engine 不由 slime 训练任务启动，而是由外部系统预先部署和管理；slime 只在训练时连接这些 engine，注册 router，并在需要时同步训练后的 actor 权重。
...
| engine 仍由 slime 启动，但需要 PD 分离、多模型、异构 server group 或 per-group overrides | [VLlm Config](vllm-config.md) |
...
slime 会请求每个 engine 的 `/server_info` 或 `/get_server_info`，推断 engine 的 GPU 数、TP/PP 信息和 worker 类型（`regular`、`prefill`、`decode`）。如果没有提供 `--vllm-router-ip/--vllm-router-port`，slime 会启动自己的 router，并把这些 external engine 注册进去。
...
- `--vllm-config`：slime 负责 engine 生命周期。你用 YAML 描述 topology，slime 启动 server group、router，并管理多模型和选择性权重更新。
...
External engine 的一个重要含义是：VLlm serving 侧不需要使用 slime 训练任务的 Python 环境、Megatron 环境或 Ray runtime。它可以运行在单独的 VLlm 容器、独立集群或其他编排系统里；slime 只依赖 HTTP endpoint、`/server_info` 信息，以及所选权重同步方式需要的通信路径。
...
- external engine 可以使用独立 VLlm 环境；不需要安装 slime 或 Megatron 训练环境。
