...
这里我们简单介绍一下脚本 [run-glm4.7-30B-A3B-8gpus.sh](https://github.com/THUDM/vime/blob/main/scripts/run-glm4.7-30B-A3B-8gpus.sh) 中的关键部分。
...
3. 开启 vLLM 支持的 MoE 优化，使用 DP attention：
...
> ⚠️ **注意**：投机解码会占用额外的 GPU 显存。如果遇到 OOM 问题，可以尝试降低 `--vllm-mem-fraction-static` 或关闭投机解码。
...
当总卡数并不能被 expert 总数（64）乘除时，可以使用 `--vllm-ep-num-redundant-experts` 来增加冗余的 expert。例如对于 24 卡的场景：
