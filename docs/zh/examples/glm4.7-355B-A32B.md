...
这里我们简单介绍一下 [run-glm4.7-355B-A32B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-glm4.7-355B-A32B.sh) 中的关键部分。
...
> ⚠️ **注意**：投机解码会额外占用 GPU 显存。如果遇到 OOM，可以尝试降低 `--vllm-mem-fraction-static` 或暂时关闭投机解码。
...
如果 rollout GPU 数与 expert 数（160）之间不能整除，可以通过 `--vllm-ep-num-redundant-experts` 增加冗余 expert。
