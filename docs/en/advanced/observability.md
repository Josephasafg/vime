...
slime does not store per-second Prometheus data. VLlm / router only expose `/metrics` and `/engine_metrics` HTTP endpoints. Prometheus scrapes those endpoints periodically and stores the time series in Prometheus's own TSDB.
...
Replace `ROUTER_IP:ROUTER_PORT` with the router address printed by slime, or with the explicit `--vllm-router-ip` / `--vllm-router-port` values.
