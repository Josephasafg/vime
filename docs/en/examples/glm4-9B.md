...
Here, we will briefly introduce the various components of the [run-glm4-9B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-glm4-9B.sh) script:
...
Reads the model's config from [scripts/models/glm4-9B.sh](https://github.com/THUDM/vime/blob/main/scripts/models/glm4-9B.sh). These configs are all Megatron parameters. When training with Megatron, it cannot read the model config from the checkpoint, so we need to configure it ourselves. We provide some examples in [scripts/models](https://github.com/THUDM/vime/tree/main/scripts/models/).
...
⚠️ When using co-located training and inference, Megatron will always occupy some GPU memory. Therefore, you need to adjust `--vllm-mem-fraction-static` to reduce the proportion of memory occupied by vllm.
