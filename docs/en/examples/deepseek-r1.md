...
Regarding parallelism, for vllm we will enable EP64, activate dp attention, and deepep. For the Megatron part, we will use TP8, PP4, EP32, and CP4.
...
This reads the model's config from [scripts/models/deepseek-v3.sh](https://github.com/THUDM/vime/blob/main/scripts/models/deepseek-v3.sh). These configs are all Megatron parameters. When training with Megatron, it cannot read the model config from the checkpoint, so we need to configure it ourselves. We provide some examples in [scripts/models](https://github.com/THUDM/vime/tree/main/scripts/models/).
