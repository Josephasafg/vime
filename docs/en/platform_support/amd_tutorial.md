...
If you are running vime on AMD's Instinct, please refer to the following materials. This tutorial will explain how to set up the development environment (Docker), use the modified ROCm dependencies, and provide an example for running the experiments. The current rocm docker only support AMD's MI300 and MI325 GPUs.


<!-- First, you need to configure the vime runtime environment according to the [Readme](../../README.md) documentation and cd to the vime project directory. -->

## Docker

You can download the prebuilt image from DockerHub: [rlsys/vime](https://hub.docker.com/r/rlsys/vime/tags).
```bash
docker pull rlsys/vime:latest
```
Or you can use the [Dockerfile.rocm](https://github.com/THUDM/vime/blob/main/docker/Dockerfile.rocm) to build it on your side.
```bash
cd docker
docker build -f Dockerfile.rocm -t rlsys/vime:latest .
```
...
Based on the [rlsys/vime](https://hub.docker.com/r/rlsys/vime/tags) image (pre-installed with vLLM and Megatron-LM):
```bash
docker run --rm -it \
  --device /dev/dri \
  --device /dev/kfd \
...
Then, download and install vime:
```bash
...
Since vime uses Megatron, and Megatron does not support loading Hugging Face checkpoints directly, we need to convert the model to the `torch_dist` format that Megatron supports.
...
⚠️ If you encounter an issue where vime cannot be found, please run `pip install -e . --no-deps` in the vime directory.


### Example: Qwen3-4B

We provide examples to use [Qwen3-4B](https://huggingface.co/Qwen/Qwen3-4B), please refer to:
- [Example: Qwen3-4B Model](https://github.com/THUDM/vime/blob/main/scripts/run-qwen3-4B-amd.sh): Just run
...
### AMD Support ###
VIME_DIR="${VIME_DIR:-/home/yushensu/projects/vime}" # Default path if not set in environment
export VIME_DIR
