...
With the help of the community, we've finally released the first version of **vime**, **v0.1.0**, just two months after it was open-sourced.

In a nutshell, this version can be summarized as follows:

> **vime v0.1.0 provides all the essential performance optimizations needed for large-scale MoE RL training.**
...
We hope to use vime v0.1.0 to demonstrate our understanding of high-performance RL training frameworks and have it become a baseline for future performance comparisons.
...
1.  **Reduce memory access through quantization**: Considering that long calibration is not feasible in RL training, vime opts for fp8 quantization.
2.  **Use deepep low-latency mode to reduce all2all latency across machines**: To work with deepep, vime recommends using blockwise quantization with fp8 to enable related vLLM configurations.
3.  **Enable Speculative Sampling**: vime allows loading any draft model for the inference part (currently, it doesn't support updating the draft model during training).
...
In addition to monitoring inference throughput, vime also monitors `perf/longest_sample_tokens_per_sec` to better understand the potential for performance optimization in the inference part.
...
With help from the vLLM community, we updated `torch_memory_saver` for vime's needs, implementing this offload solution.
...
Parameter update is another special step in RL training. For this, vime v0.1.0 provides the best optimization solution for scenarios where training and inference are in different processes. This work was heavily optimized by Biao He. I recommend reading his blog post:

  - [Efficient Reinforcement Learning Training - Optimizing Weight Synchronization in vime](https://hebiao064.github.io/rl-weight-sync)

Currently, vime can complete weight synchronization for a GLM4.5 355B-A32B model with bf16 weights in **48s** and complete fp8 blockwise quantization + parameter update in **100s** (the fp8 branch is still being optimized).

-----

## Training Optimization

For the pure training part of vime, we believe Megatron already provides ample optimizations, so our main focus was to **ensure compatibility with all of Megatron's parallel strategies**.

During this adaptation, we found an interesting bugfix: we discovered that when vLLM enabled mtp, the Megatron part couldn't start DeepEP. It turned out that when mtp is enabled, vLLM disables the overlap schedule, which causes a certain metadata communication to use nccl instead of gloo after being offloaded to the CPU, and this conflicts with DeepEP.

-----

## Performance Optimization Check List

Since the release of vime, I've often been asked about its performance comparison with other frameworks.

My understanding of benchmarks is that they should not be used as a weapon for frameworks to attack each other, but rather as a **tool for identifying gaps**. To that end, we will gradually release performance benchmarks that vime focuses on to improve ourselves.
...
I also believe that before running benchmarks, you can analyze a framework's focus on performance from a qualitative perspective. Here's a basic feature check list for optimizations:
  ...
  - Can the internal vllm `mem_fraction` or vllm `gpu_utilization` be adjusted to over 0.7? (Ensures KV Cache space)
...
vime v0.1.0 has made preliminary attempts at all the above optimizations, and there's still a lot of room for improvement. We hope this version can serve as a baseline for future vime versions or for performance comparisons between different frameworks. We also welcome all friends who share our pursuit of performance to try out vime and join the vime community\!
...
vime v0.1.0 adds **end-to-end CI**: we run single-machine GLM4 9B and Qwen3 30B-A3B training for each PR, ensuring correctness through strict checks. For example, we explicitly require:
...
This is a brief introduction to vime v0.1.0. I hope it sparks your curiosity about vime and that it can be helpful in your work.

Everyone is welcome to join the vime community. Let's work together to build an open RL Infra and contribute to RL scaling\!