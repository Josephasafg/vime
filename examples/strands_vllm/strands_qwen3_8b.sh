...
# Qwen3-8B Training with Strands-vLLM
...
VLLM_ARGS=(
   ...
   --vllm-mem-fraction-static 0.4
   # Note: strands-vllm handles tool parsing internally (HermesToolCallParser)
   # No need for --vllm-tool-call-parser
)
...
CUSTOM_ARGS=(
   --custom-generate-function-path examples.strands_vllm.generate_with_strands.generate
   --custom-rm-path examples.strands_vllm.generate_with_strands.reward_func
)
