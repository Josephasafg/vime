...
- **Server groups within a model can be heterogeneous.** Different groups can have different TP sizes, worker types (prefill/decode/regular), and vLLM server argument overrides.
  ...
  - name: <model_name>              # Required. Unique identifier for this model.
    ...
    server_groups:                  # Required. List of server group configurations.
      - worker_type: <type>         # Required. One of: regular, prefill, decode, placeholder.
        ...
        overrides: <dict>           # Optional. vLLM ServerArgs field overrides.
```
...
| `overrides` | `dict` | `{}` | vLLM `ServerArgs` field overrides. Applied on top of `--vllm-*` CLI args with highest priority. |
...
> **Note:** PD disaggregation uses vLLM Model Gateway (sgl-router) with `pd_disaggregation=True`.
...
Each model in the config gets its own independent router (vLLM Model Gateway by default).
...
vime automatically assigns each sample a unique `session_id` (stored in `sample.session_id`). When the router policy is `consistent_hashing`, this ID is passed as the `X-SMG-Routing-Key` header, and vLLM Model Gateway uses it to deterministically route all turns of the same session to the same worker.
...
2. On each request, vime passes `X-SMG-Routing-Key: <session_id>` in the HTTP header
3. vLLM Model Gateway's consistent hashing policy maps this key to a specific worker
