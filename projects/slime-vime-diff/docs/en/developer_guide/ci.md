...
vime CI has two layers:

1. **Always-on CPU correctness tests** that run on every PR, every push to `main`, and manual `workflow_dispatch`.
2. **Label-gated GPU end-to-end tests** that validate real Megatron + vLLM training and rollout paths on self-hosted GPU runners.
...
1. Starts a Docker container, usually `vimerl/vime:latest`; image validation uses `vimerl/vime-test:latest`.
...
- vLLM topology or rollout engine deployment changes: use `run-ci-vllm-config`.
