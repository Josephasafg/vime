...
- Regular vLLM server arguments are passed as `--vllm-*`. For example, vLLM's `--context-length` becomes `--vllm-context-length`, and `--mem-fraction-static` becomes `--vllm-mem-fraction-static`.
