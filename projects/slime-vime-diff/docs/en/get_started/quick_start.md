...
- For scenarios where Docker is not convenient, please refer to [build_conda.sh](https://github.com/THUDM/vime/blob/main/build_conda.sh);
...
docker pull vimerl/vime:latest

# Start the container
docker run --rm --gpus all --ipc=host --shm-size=16g \
  --ulimit memlock=-1 --ulimit stack=67108864 \
  -it vimerl/vime:latest /bin/bash
```
...
This part of parameters is used to configure vLLM inference service.
- `--rollout-num-gpus-per-engine`: Basically equivalent to vLLM's `tp_size`.
...
> ⚠️ **Note**:
> vime uses `sgl-router` to schedule multiple vLLM Servers. Without enabling DP Attention, `dp_size` will be calculated through `rollout-num-gpus/rollout-num-gpus-per-engine`.
...
> ⚠️ **Note**:
> In training-inference integration mode, Megatron will occupy a certain amount of GPU memory before it can be offloaded after initialization. You need to adjust the `--vllm-mem-fraction-static` parameter to reduce vLLM's GPU memory usage ratio to avoid insufficient GPU memory. We usually recommend 0.8.
...
2. **Call Model to Generate Action**: In each round of the loop, call vLLM service to let the model generate the next action (such as `<search>query</search>`) based on the current conversation history.
