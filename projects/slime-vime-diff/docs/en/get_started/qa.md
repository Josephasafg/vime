...
8.  **What should I do if the vllm component shows a `Max retries exceeded with url: /get_model_info (Caused by NewConnectionError)` error?**

    This issue primarily stems from port conflicts caused by multiple vllm servers running on a single machine. We are currently working with the vllm team to resolve this. A temporary workaround is to minimize the number of vllm servers on a single machine, for example, by setting `tp=8`.
...
11. **Sglang shows an `an illegal memory access was encountered` error.**
    ...
    According to [vLLM documentation](https://docs.vllm.io/references/faq.html), this could be an OOM error. Consider reducing the value of `--vllm-mem-fraction-static`.
