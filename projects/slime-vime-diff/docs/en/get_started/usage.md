...
There are four main parameters for cluster resource allocation:
  ...
  - `--rollout-num-gpus-per-engine`: The number of GPUs per inference engine. This parameter is similar to vLLM's `tp_size`. When performing multi-node serving, this value should be the total number of GPUs. For example, if serving one model with 2 nodes and 16 GPUs, this value should be 16.
    The reason for not using a parameter like `--vllm-tp-size` is that we might consider supporting vLLM's `dp_size` parameter in the future, which means an engine could contain multiple vLLM servers (currently, only `--vllm-dp-size` under the `--vllm-enable-dp-attention` condition is supported).
...
Unlike tools such as vLLM, vLLM, or Hugging Face Trainer, Megatron cannot directly read Hugging Face checkpoints. Instead, the user must configure the parameters for the model to be trained and load Megatron's own checkpoint format.
...
vime supports customizing data generation (rollout) to various degrees.
  ...
  - By default, it uses the `generate_rollout` function from [vime/rollout/vllm_rollout.py](https://github.com/THUDM/vime/blob/main/vime/rollout/vllm_rollout.py) for data generation. This file implements an asynchronous (asyncio) data generation flow based on vLLM and supports features like dynamic sampling and partial rollout.

  - You can completely replace the `generate_rollout` in vllm\_example.py by using the `--rollout-function-path` parameter. You just need to ensure that the function signature passed via `--rollout-function-path` is as follows:
    ...
    Where:
      ...
      - The returned `Sample` type is defined in [vime/utils/types.py](https://github.com/THUDM/vime/blob/main/vime/utils/types.py). When implementing, you need to ensure the following fields are correctly set:
  ...
  - In some cases, you may only need to replace the data generation logic. You can do this using `--custom-generate-function-path`. A simplified implementation of this function is as follows:
    ...
    async def generate(args, sample: Sample, sampling_params) -> Sample:
        ...
        output = await post(
            f"http://{args.vllm_router_ip}:{args.vllm_router_port}/generate",
            {
                "text": sample.prompt,
    ...
    For a more complete version, please refer to [vime/rollout/vllm_rollout.py](https://github.com/THUDM/vime/blob/main/vime/rollout/vllm_rollout.py).

  - Sometimes, you may also need to support a custom reward model. This can be configured by setting `--custom-rm-path`.

## How to Use vLLM

vime implements a server-based engine using vLLM via the `HttpServerEngineAdapter` as an intermediary.

### Parameter Configuration

vime incorporates almost all vLLM parameters by using vLLM's `ServerArgs.add_cli_args`. When setting an vLLM parameter, you need to add the `--vllm-` prefix. For example:

  - In co-located training and inference, you often need to limit `--mem-fraction-static`. This parameter should be changed to `--vllm-mem-fraction-static`.
  - During training, if you want vLLM to infer beyond the maximum context length specified in the Hugging Face checkpoint's `config.json`, you need to use `--context-length`, which becomes `--vllm-context-length` in vime.
  - For multi-node large EP inference, you might need `--ep-size`, `--enable-dp-attention`, `--dp-size`, `--moe-a2a-backend deepep`, etc. These can be passed as `--vllm-ep-size`, `--vllm-enable-dp-attention`, `--vllm-dp-size`, and `--vllm-moe-a2a-backend deepep` respectively.

Some parameters related to vime's resource scheduling are configured by vime itself, for example:

  - `--tp-size` in vime is set using `--rollout-num-gpus-per-engine`.
  - `--model-path` in vime is set using `--hf-checkpoint`.

The way vLLM parameters are integrated into vime can be found in [vime/backends/vllm_utils/arguments.py](https://github.com/THUDM/vime/blob/main/vime/backends/vllm_utils/arguments.py).

### How to Use the Router

vime uses [vllm-router](https://github.com/sgl-project/vllm/tree/main/sgl-model-gateway) to manage the vLLM servers during the training process. You can configure the address of the [vllm-router](https://github.com/sgl-project/vllm/tree/main/sgl-model-gateway) using `--vllm-router-ip` and `--vllm-router-port`. If not configured, a router will be started by default within the cluster.

After starting, all vLLM servers will register with the router via the `/add_worker` endpoint. When actually generating data, you only need to send HTTP requests to the router, which will perform load balancing and forward the requests to the servers.
...
- `overrides`: Dict of vLLM `ServerArgs` field overrides applied on top of `--vllm-*` CLI args
