...
A native example of this pattern is [`examples/search-r1`](../../../examples/search-r1/), which adds search-augmented multi-turn generation via `--custom-generate-function-path` while keeping vime's default `vllm_rollout` outer loop. See also [`examples/multi_agent`](../../../examples/multi_agent/README.md) for a `--rollout-function-path`-based multi-agent pattern and [`examples/fully_async`](../../../examples/fully_async/README.md) for long-tail agentic generation.
...
| `--use-rollout-routing-replay` | R3: Replay routing from rollout during training. Supported by vime's default `vllm_router` path. ([arXiv:2510.11370](https://arxiv.org/abs/2510.11370)) |
