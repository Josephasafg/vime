...
Here, we briefly introduce the key parts in the [run-glm4.7-355B-A32B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-glm4.7-355B-A32B.sh) script.
...
If your rollout GPU count does not divide the expert count cleanly, you can use `--vllm-ep-num-redundant-experts` to add redundant experts.
...
VLLM_ARGS=(
   ...
   --vllm-moe-dense-tp-size 1
   --vllm-enable-dp-lm-head
   --vllm-cuda-graph-bs 1 2 4 8 $(seq 16 8 128)
