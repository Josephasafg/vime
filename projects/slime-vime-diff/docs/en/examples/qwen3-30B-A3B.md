...
Here, we will briefly introduce the MoE-related parts in the [run-qwen3-30B-A3B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-qwen3-30B-A3B.sh) script.
...
3.  Enable MoE optimization supported by vLLM. The current configuration is ep8:
...
In addition, you can make the following changes:
  ...
  - When the total number of GPUs is not a multiple or divisor of the total number of experts, you can use `--vllm-ep-num-redundant-experts` to add redundant experts. For example, in a 24-GPU scenario, you can configure it as follows:
