...
rollout 过程中，vime 会定期向所有 vLLM server 发送 heartbeat 请求（`/health_generate`）。如果 heartbeat timeout，异常 vLLM server 会被停止。当前 rollout round 完成后，vime 会重启 server，并在其继续服务后续 rollout 请求前更新到正确参数。
