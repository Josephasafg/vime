  ...
  - name: <model_name>              # 必填。模型的唯一标识符。
    ...
    server_groups:                  # 必填。服务器组配置列表。
      - worker_type: <type>         # 必填。可选：regular、prefill、decode、placeholder。
        ...
        overrides: <dict>           # 可选。vLLM ServerArgs 字段覆盖。
```
...
> **注意：** PD 分离使用 vLLM Model Gateway (sgl-router)，并设置 `pd_disaggregation=True`。
...
每个模型都有独立的 router（默认使用 vLLM Model Gateway）。
...
vime 自动为每个 sample 分配一个唯一的 `session_id`（存储在 `sample.session_id` 中）。当 router 策略为 `consistent_hashing` 时，该 ID 通过 `X-SMG-Routing-Key` header 传递，vLLM Model Gateway 使用它将同一会话的所有轮次确定性地路由到同一个 worker。
...
2. 每次请求时，vime 在 HTTP header 中传递 `X-SMG-Routing-Key: <session_id>`
3. vLLM Model Gateway 的 consistent hashing 策略将该 key 映射到特定的 worker
