...
如果要使用单独训练的 draft model（例如 [SpecForge](https://docs.vllm.ai/SpecForge/) 训练的），还需要额外设置：

```bash
--vllm-speculative-draft-model-path /your/draft/model/path
```

详细参数含义及配置方法，请参考 vLLM 的 speculative decoding [文档](https://docs.vllm.ai/advanced_features/speculative_decoding.html)
