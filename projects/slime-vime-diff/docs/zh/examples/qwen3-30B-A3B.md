...
这里我们简单介绍一下脚本 [run-qwen3-30B-A3B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-qwen3-30B-A3B.sh) 中与 MoE 相关的部分。
...
3. 开启 vllm 支持的 moe 优化，当前配置为 ep8：
...
- 当总卡数并不能被 expert 总数乘除时，可以使用 `--vllm-ep-num-redundant-experts` 来增加冗余的 expert，例如对于 24 卡的场景，可以配置：
