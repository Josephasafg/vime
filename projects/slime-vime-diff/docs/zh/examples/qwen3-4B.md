...
这里我们简单介绍一下脚本 [run-qwen3-4B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-qwen3-4B.sh) 中的各个组成部分：
...
从 [scripts/models/qwen3-4B.sh](https://github.com/THUDM/vime/blob/main/scripts/models/qwen3-4B.sh) 读取模型的 config。这些 config 都是 megatron 的参数。在使用 megatron 进行训练的时候，megatron 无法从 ckpt 中读取模型 config，需要我们自行配置。我们在 [scripts/models](https://github.com/THUDM/vime/tree/main/scripts/models/) 中提供了一些样例。
...
⚠️  vime 会用 sgl-router 来调度多个 vllm server，在不开启 dp attention 的情况下不支持 `dp_size`。
...
2. 使用 `--vllm-cuda-graph-bs`，即 vllm 原生的 `--cuda-graph-bs`, 增大 vllm 初始化的 cuda graph 数量，例如：
