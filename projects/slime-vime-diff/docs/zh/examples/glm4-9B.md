...
这里我们简单介绍一下脚本 [run-glm4-9B.sh](https://github.com/THUDM/vime/blob/main/scripts/run-glm4-9B.sh) 中的各个组成部分：
...
从 [scripts/models/glm4-9B.sh](https://github.com/THUDM/vime/blob/main/scripts/models/glm4-9B.sh) 读取模型的 config。这些 config 都是 megatron 的参数。在使用 megatron 进行训练的时候，megatron 无法从 ckpt 中读取模型 config，需要我们自行配置。我们在 [scripts/models](https://github.com/THUDM/vime/tree/main/scripts/models/) 中提供了一些样例。
...
⚠️  在训推一体的训练时，megatron 始终会占据一些显存，所以需要通过调整 `--vllm-mem-fraction-static` 来降低 vllm 占据的显存比例。
