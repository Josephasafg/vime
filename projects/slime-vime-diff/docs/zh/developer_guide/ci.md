...
2. **通过 label 触发的 GPU end-to-end 测试**：在自托管 GPU runner 上验证真实的 Megatron + vLLM training/rollout 路径。
...
1. 启动 Docker container，通常使用 `vimerl/vime:latest`；镜像验证使用 `vimerl/vime-test:latest`。
...
- vLLM topology 或 rollout engine deployment 改动：使用 `run-ci-vllm-config`。
