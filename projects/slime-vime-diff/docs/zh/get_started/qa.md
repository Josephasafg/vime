...
1. **vllm 部分出现 `Max retries exceeded with url: /get_model_info (Caused by NewConnectionError` 的问题怎么办？**

   这个问题主要来源于单机内多个 vllm server 导致的端口冲突，目前我们仍在和 vllm 团队一起解决这个问题。一个临时的缓解方案是尽可能减少单机内的 vllm server 数量，例如设置 tp=8。
...
1. **vllm 出现 an illegal memory access was encountered**
   ...
   根据 vllm 的文档（https://docs.vllm.ai/references/troubleshooting.html），有可能是 OOM 了，可以考虑缩小 `--vllm-mem-fraction-static`。
