...
- `--rollout-num-gpus-per-engine`：每个 inference engine 有多少卡，这个参数会比较像 vllm 的 `tp_size`，也就是在进行多机 serving 的时候，这个数值应该是总卡数，例如 2 机 16 卡 serving 一个模型，这里的值应该是 16。
  ...
  这里不像其他的 vllm 参数那样引入 `--vllm-tp-size` 是因为未来也许会考虑支持 vllm 的 dp_size 参数，也就是一个 engine 里面其实是有多个 vllm server 的（目前只支持 `--vllm-enable-dp-attention` 情况下的 `--vllm-dp-size`）。
...
megatron 与 vllm, vllm 或者 huggingface trainer 之类的工具不同，他不能直接读取 huggingface ckpt，而是需要用户配置好要训练的模型的参数，并且加载 megatron 自己的 ckpt。
...
- 默认会使用 [vime/rollout/vllm_rollout.py](https://github.com/THUDM/vime/blob/main/vime/rollout/vllm_rollout.py) 中的 `generate_rollout` 函数进行数据生成。这个文件中实现了基于 vllm 的异步（asyncio）数据生成流程，并支持了例如 dynamic sampling，partial rollout 等功能；

- 可以通过 `--rollout-function-path` 参数，完全替换 vllm_rollout.py 中的 `generate_rollout`，只需要保证 `--rollout-function-path` 传入的函数签名满足：
  ...
  -  返回的 `Sample` 类型见 [vime/utils/types.py](https://github.com/THUDM/vime/blob/main/vime/utils/types.py)，在实现时，需要保证
...
- 在一些情况下，可能只需要替换数据生成的逻辑，那么使用 `--custom-generate-function-path` 进行替换即可，这个函数一个简化版实现如下：
  ...
  ```
   ...
   更完备的版本请查看 [vime/rollout/vllm_rollout.py](https://github.com/THUDM/vime/blob/main/vime/rollout/vllm_rollout.py)。

- 有的时候，我们还需要支持自定义的 reward model，可以通过配置 `--custom-rm-path` 来进行配置。

## vllm 使用方法

vime 通过 `HttpServerEngineAdapter` 作为中介，实现了基于 vllm 的 server based engine。

### 参数配置

vime 通过引入 vllm 的 `ServerArgs.add_cli_args`，从而引入了几乎所有的 vllm 参数，在设置一个 vllm 参数的时候，需要在参数前加上 `--vllm` 的前缀，例如：

- 在训推一体的训练时，往往需要限制 `--mem-fraction-static`，这个参数需要转变为 `--vllm-mem-fraction-static`；
- 在训练中，希望 vllm 能推理超过 huggingface checkpoint 的 `config.json` 中标识的最长 context length，需要使用 `--context-length`，那么在 vime 中需要使用 `--vllm-context-length`；
- 在进行多机大 ep 推理的时候，需要 `--ep-size`、`--enable-dp-attention`、`--dp-size`、`--moe-a2a-backend deepep` 等，则可以对应地传入 `--vllm-ep-size`、`--vllm-enable-dp-attention`、`--vllm-dp-size`、`--vllm-moe-a2a-backend deepep` 。

有部分参与和 vime 的资源调度相关，会由 vime 自行配置，例如：

- `--tp-size` 在 vime 中会使用 `--rollout-num-gpus-per-engine`
- `--model-path` 在 vime 中会使用 `--hf-checkpoint`

vllm 参数引入 vime 的方式可以参考 [vime/backends/vllm_utils/arguments.py](https://github.com/THUDM/vime/blob/main/vime/backends/vllm_utils/arguments.py)。

### router 使用方法

vime 会用 [vllm-router](https://github.com/sgl-project/vllm/tree/main/sgl-router) 来管理训练过程中的 vllm server。可以通过 `--vllm-router-ip` 与 `--vllm-router-port` 来配置 [vllm-router](https://github.com/sgl-project/vllm/tree/main/sgl-router) 的地址。如果不进行配置，则会在集群中默认启动一个 router。

所有的 vllm server 在启动后，会通过 `/add_worker` 申请加入 router。在实际进行数据生成的时候，只需要向 router 发送 http 请求，router 会进行 load balancing 操作，将请求转发给 server 们。

当通过 `--vllm-router-ip` 与 `--vllm-router-port` 来配置传入一个外部的 router，此时 vime 不再会在内部启动一个 router，而是会把所有的 server 都注册在这个外部 router 上。这时可以利用这个外部的 router 地址来实现更复杂的数据生成流程。注意 router 是支持 openai compatible api 的。
...
- `overrides`：vLLM `ServerArgs` 字段覆盖字典，会叠加在 `--vllm-*` CLI 参数之上
...
vime 通过直接引入 `from megatron.training.arguments import parse_args` 引入了当前环境中 megatron 的所有参数。如果当前使用的 megatron 有在 `parse_args` 之外的参数，可以通过像 [train.py](https://github.com/THUDM/vime/blob/main/train.py) 中传入参数来进行配置，例如：
