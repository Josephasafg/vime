# vime on NVIDIA DGX Spark (GB10) — Porting Notes
...
vime's published Docker images (`vimerl/vime:*`) are x86_64-only. The arm64 base
it derives from (`vimerl/vllm:v0.5.9`) ships with CUDA 12.9 and stock PyTorch
...
This port rebases vime on `nvcr.io/nvidia/vllm:26.03-py3` (arm64), which ships:
...
| 11 | vime `train.py --help`: `'tuple' object has no attribute 'strip'` | Typo in `vime/utils/arguments.py:1073`: `help=("string",)` (trailing comma → tuple) instead of `help=("string")` | Remove trailing comma — simple one-line vime fix, upstream-able |
| 12 | `vllm_router` x86_64-only wheel from `zhuzilin/sgl-router` fork | vime Dockerfile pins `zhuzilin/sgl-router` release (no arm64 builds); vime's `'vime' in version` assertion is only in `wandb_utils.py` (non-critical path) | Install upstream `vllm-router==0.3.2` from PyPI (has arm64 wheel). Accept wandb path fallback |
...
| 14 | `megatron.training` not importable after `pip install -e Megatron-LM` | Megatron-LM's setup.py only packages `megatron-core`; `megatron.training`, `megatron.rl`, `megatron.legacy` are sibling dirs meant to be on `PYTHONPATH` | `export PYTHONPATH=/root/src/Megatron-LM:$PYTHONPATH` (vime docs confirm this) |
...
- [x] M3: vime installed with megatron.patch applied; `train.py --help` prints 3714-line arg list
- [x] M4: End-to-end RL loop (Qwen2.5-0.5B + dapo-math-17k + GRPO colocated on 1 GB10 GPU) runs clean in 2m10s

Reproducible image: `vime-gb10:m4-success` (36.3 GB, committed post-smoke)
