...
vime is developed as RL infrastructure, where "the script runs" is not enough. The project maintains CPU unit tests, contract tests for customization hooks, and GPU end-to-end tests covering dense and MoE models, Megatron training paths, vLLM deployment configurations, checkpointing, numerical precision, async rollout, OPD, PPO-style workflows, and debug rollout-then-train replay.
...
- Our vision: [vime: An vLLM-Native Post-Training Framework for RL Scaling](https://lmsys.org/blog/2025-07-09-vime/).
...
- v0.1.0 release note: [v0.1.0: Redefining High-Performance RL Training Frameworks](https://thudm.github.io/vime/blogs/release_v0.1.0.html)

## Table of Contents

- [Why This Design Matters](#why-this-design-matters)
- [Production Validation](#production-validation)
- [Native Engine Pass-Through and vLLM Deployment](#native-engine-pass-through-and-vllm-deployment)
- [Correctness, Stability, and CI](#correctness-stability-and-ci)
- [Architecture Overview](#architecture-overview)
- [Quick Start](#quick-start)
- [Ecosystem Built on vime](#ecosystem-built-on-vime)
...
- **rollout (vLLM + router)**: Generates new data (including rewards/verifier outputs) and stores it in the Data Buffer. Custom generate functions can wrap this with multi-turn loops, tool calls, environment/sandbox interaction, and verifier-based reward.
...
1.  **Megatron arguments**: vime reads Megatron arguments directly. You can configure Megatron by passing arguments like `--tensor-model-parallel-size 2`.
2.  **vLLM arguments**: All arguments for the installed vLLM are supported through pass-through. These arguments must be prefixed with `--vllm-`. For example, `--mem-fraction-static` should be passed as `--vllm-mem-fraction-static`.
3.  **vime-specific arguments**: Please refer to: [vime/utils/arguments.py](vime/utils/arguments.py)
...
- Special thanks to the following projects & communities: vLLM, Megatron‑LM, mbridge, OpenRLHF, veRL, Pai-Megatron-Patch and others.
- To quote vime, please use:

```bibtex
@misc{vime_github,
  author       = {Zilin Zhu and Chengxing Xie and Xin Lv and vime Contributors},
  title        = {vime: An LLM post-training framework for RL Scaling},
  year         = {2025},
  howpublished = {\url{https://github.com/THUDM/vime}},
