...
class vLLMEngine(RayActor):
    ...
    def health_generate(self, timeout: float = 5.0) -> bool:
        """Run /health_generate on the underlying vLLM HTTP server.
...
def _compute_server_args(
    ...
    for attr in server_arg_fields:
        if worker_type == "decode" and attr.name == "enable_hierarchical_cache":
            continue
        if hasattr(args, f"vllm_{attr.name}") and attr.name not in kwargs:
            kwargs[attr.name] = getattr(args, f"vllm_{attr.name}")
