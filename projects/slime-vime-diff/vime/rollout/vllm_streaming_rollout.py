...
Drop-in alternative to :func:`vime.rollout.vllm_rollout.generate` that
consumes vllm's SSE stream incrementally instead of awaiting one final JSON
...
partial-rollout buffer hand-off) is still owned by ``vllm_rollout``; this
file only replaces the inner HTTP call.

vllm's default streaming output is cumulative — server-side
...
``--incremental-streaming-output`` on the vllm server, the text/output_ids
...
async def generate_streaming(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    # Snapshot pre-call sample state. vllm's SSE chunks are cumulative
    ...
    with trace_span(
        sample, "vllm_generate_stream", attrs={"max_new_tokens": sampling_params["max_new_tokens"]}
    ) as span:
