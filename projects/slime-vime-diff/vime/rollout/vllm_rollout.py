...
async def generate(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    """Generate using traditional vLLM router with token-based workflow"""
    if args.ci_test:
    ...
    if images:
        ...
        # For single-turn multimodal requests, send text so vLLM expands the
    ...
    # Use session_id for consistent hashing routing (vLLM Model Gateway)
    headers = None
    ...
    with trace_span(sample, "vllm_generate", attrs={"max_new_tokens": sampling_params["max_new_tokens"]}) as span:
        ...
        span.update(build_vllm_meta_trace_attrs(output["meta_info"]))
