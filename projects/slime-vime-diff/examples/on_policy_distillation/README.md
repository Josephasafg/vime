...
- **Two teacher modes**:
  - **vllm**: Teacher runs on an external vLLM server, teacher log-probs are obtained during rollout.
...
- `run-qwen3-8B-opd.sh` launches an vLLM teacher server, then submits a Ray job that runs `train.py`.
