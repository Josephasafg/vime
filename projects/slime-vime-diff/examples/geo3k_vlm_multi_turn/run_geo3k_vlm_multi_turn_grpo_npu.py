...
def execute():
    ...
    vllm_args = (
        ...
        f"--vllm-cuda-graph-bs {' '.join(map(str, [4, 8] + list(range(16, 257, 8))))} "
        "--vllm-device npu "
        "--vllm-disable-radix-cache "
        "--vllm-chunked-prefill-size 32768 "
        "--vllm-max-prefill-tokens 4000 "
        "--vllm-max-total-tokens 327680 "
    )
